...
#define DISALLOW_COPY_WVP_ASSIGN(TypeName) \
...
private:
    ...
    ::HWVPLE _hFile;
    DISALLOW_COPY_WVP_ASSIGN(Win32SequentialFile);
};
...
private:
    ...
    DISALLOW_COPY_WVP_ASSIGN(Win32RandomAccessFile);
};
...
private:
    ...
    bool _MapNewRegion();
    DISALLOW_COPY_WVP_ASSIGN(Win32MapFile);
    BOOL _Init(LPCWSTR Path);
};
...
private:
    ...
    HWVPLE _hFile;
    std::string _filename;
    DISALLOW_COPY_WVP_ASSIGN(Win32FileLock);
};
...
private:
    ...
    DISALLOW_COPY_WVP_ASSIGN(Win32Logger);
};
...
BOOL Win32RandomAccessFile::_Init( LPCWSTR path )
{
    ...
    if(!_hFile)
        ...
        FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RWVPOM_ACCESS,NULL);
    if(!_hFile || _hFile == INVALID_HWVPLE_VALUE )
        _hFile = NULL;
    else
        bRet = TRUE;
    return bRet;
...
{
    if (_hFile != INVALID_HWVPLE_VALUE) { 
        Win32MapFile::Close();
    }
}
...
Status Win32Env::GetChildren(const std::string& dir, std::vector<std::string>* result)
{
	...
	::HWVPLE hFind = ::FindFirstFileW(wpath.c_str() ,&wfd);
    if(hFind && hFind != INVALID_HWVPLE_VALUE){
        BOOL hasNext = TRUE;
        std::string child;
        while(hasNext){
...
Status Win32Env::GetFileSize( const std::string& fname, uint64_t* file_size )
{
    ...
    HWVPLE file = ::CreateFileW(wpath.c_str(),
