...
int nAnonymizeWavepayAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wavepay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wavepay
    // Mac: ~/Library/Application Support/Wavepay
    // Unix: ~/.wavepay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wavepay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "wavepay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "wavepayd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong WAVEPAY will not work properly.");
