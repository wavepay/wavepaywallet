...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(addr.GetPort() != 5279) return;
            if(!isIPV4) return;
        } else if(addr.GetPort() == 5279) return;
        ...
        if(fAcceptable){
            ...
            // should be at least not earlier than block when 20000 TansferCoin tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            GetTransaction(vin.prevout.hash, tx, hashBlock);
            map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(hashBlock);
           if (mi != mapBlockIndex.end() && (*mi).second)
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 20000 TansferCoin tx -> 1 confirmation
