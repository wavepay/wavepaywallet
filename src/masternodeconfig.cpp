...
bool CMasternodeConfig::read(boost::filesystem::path path) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                LogPrintf("InvalidWVPaddress in masternode.conf line: %s\n", line.c_str());
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 5279) {
                LogPrintf("Invalid port detected in masternode.conf: %s (must be 5279 for mainnet)\n", line.c_str());
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 5279) {
            LogPrintf("Invalid port detected in masternode.conf: %s (5279 must be only on mainnet)\n", line.c_str());
            streamConfig.close();
            return false;
        }
