...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)(GetProofOfStakeReward(0)/COIN);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Wavepay is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Wavepay is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Wavepay is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Wavepay is downloading blocks...");
