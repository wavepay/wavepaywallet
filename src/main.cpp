...
static const int64_t nTargetSpacingWork = 180; // 3 min PoW block spacing
...
const string strMessageMagic = "Wavepay Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    // to MAX_STWVPARD_TX_SIZE mitigates CPU exhaustion attacks.
    unsigned int sz = tx.GetSerializeSize(SER_NETWORK, CTransaction::CURRENT_VERSION);
    if (sz >= MAX_STWVPARD_TX_SIZE) {
        reason = "tx-size";
        return false;
    }
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1,1,1), pindexBest, false, false, STWVPARD_SCRIPT_VERIFY_FLAGS))
        {
        ...
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1,1,1), pindexBest, false, false, MWVPATORY_SCRIPT_VERIFY_FLAGS))
        {
            return error("AcceptToMemoryPool: : BUG! PLEASE REPORT THIS! ConnectInputs failed against MWVPATORY but not STWVPARD flags %s", hash.ToString());
        }
    }
                         ...
                         bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        // Check against previous transactions
        // This is done last to help prevent CPU exhaustion denial-of-service attacks.
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1,1,1), pindexBest, true, false, STWVPARD_SCRIPT_VERIFY_FLAGS, false))
        {
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t _nTargetTimespan = CountBlocks * nTargetSpacingWork;
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if(fValidateSig)
            {
                ...
                if (!(fBlock && !IsInitialBlockDownload()))
                {
                    ...
                    {
                        if (flags & STWVPARD_NOT_MWVPATORY_VERIFY_FLAGS) {
                            ...
                            // non-upgraded nodes.
                            if (VerifySignature(txPrev, *this, i, flags & ~STWVPARD_NOT_MWVPATORY_VERIFY_FLAGS, 0))
