...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeWavepayAmount"))
        settings.setValue("nAnonymizeWavepayAmount", 1000);
    nAnonymizeWavepayAmount = settings.value("nAnonymizeWavepayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeWavepayAmount"))
        SoftSetArg("-anonymizewavepayamount", settings.value("nAnonymizeWavepayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeWavepayAmount:
            return QVariant(nAnonymizeWavepayAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeWavepayAmount:
            nAnonymizeWavepayAmount = value.toInt();
            settings.setValue("nAnonymizeWavepayAmount", nAnonymizeWavepayAmount);
            emit AnonymizeWavepayAmountChanged(nAnonymizeWavepayAmount);
            break;
        default:
            break;
        }
