...
{
    resize(900, 500);
    setWindowTitle(tr("Wavepay") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("wavepay");
    setStyleSheet("#wavepay { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
    ...
    statusBar()->setStyleSheet("#statusBar { color: #ffffff; background-color: qradialgradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgb(0, 0, 255), stop:1 rgba(255, 255, 255, 255));  }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction = new QAction(QIcon(":/icons/send_0"), tr("&Send"), this);
    sendCoinsAction->setToolTip(tr("Send coins to a Wavepay address"));
    ...
    masternodeManagerAction->setToolTip(tr("Show Master Nodes status, Roi, Map and configure your nodes."));
    ...
    marketsAction = new QAction(QIcon(":/icons/market"), tr("&Markets"), this);
    marketsAction->setToolTip(tr("View Altcoin Markets"));
    marketsAction->setCheckable(true);
    tabGroup->addAction(marketsAction);

   // miningAction = new QAction(QIcon(":/icons/mining"), tr("&Mining"), this);
   // miningAction->setToolTip(tr("Mining"));
   ...
   // connect(TradingAction, SIGNAL(triggered()), this, SLOT(gotoTradingPage()));
    ...
    connect(marketsAction, SIGNAL(triggered()), this, SLOT(gotoMarkets()));
    // connect(miningAction, SIGNAL(triggered()), this, SLOT(gotoMining()));
    ...
    aboutAction = new QAction(QIcon(":/icons/aboutwavepay"), tr("&About Wavepay"), this);
    aboutAction->setToolTip(tr("Show information about Wavepay"));
    aboutAction->setMenuRole(QAction::AboutRole);
    websiteAction = new QAction(QIcon(":/icons/website"), tr("&Website"), this);
    websiteAction->setToolTip(tr("Visit Website Wavepay"));
    facebookAction = new QAction(QIcon(":/icons/facebook"), tr("&Facebook"), this);
    facebookAction->setToolTip(tr("Visit Facebook Wavepay"));
    telegramAction = new QAction(QIcon(":/icons/telegram"), tr("&Telegram"), this);
    telegramAction->setToolTip(tr("Visit Telegram Wavepay"));
    discordAction = new QAction(QIcon(":/icons/discord"), tr("&Discord"), this);
    discordAction->setToolTip(tr("Visit Discord Wavepay"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for Wavepay"));
...
void BitcoinGUI::createMenuBar()
{
    ...
    QMenu *media = appMenuBar->addMenu(tr("&Media"));
    media->addAction(websiteAction);
    media->addAction(facebookAction);
    media->addAction(telegramAction);
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #ffffff; font-weight:bold;} QToolButton:hover { background-color: rgb(146, 222, 237) } QToolButton:checked { background-color: rgb(146, 222, 237) } QToolButton:pressed { background-color: rgb(146, 222, 237) } #tabs { color: #ffffff; background-color: qradialgradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgb(0, 0, 255), stop:1 rgba(255, 255, 255, 255));  }");
    ...
    toolbar->addAction(marketsAction);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Wavepay client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Wavepay client"));
...
{
	QString web = "https://wavepay.org";
	QDesktopServices::openUrl(QUrl(web));
}

void BitcoinGUI::facebookClicked()
{
	QString faceb = "https://www.facebook.com/Wavepay-1581049692004221";
	QDesktopServices::openUrl(QUrl(faceb));
}

void BitcoinGUI::telegramClicked()
{
...
{
	QString discord = "https://discord.gg/vJZRsHK";
	QDesktopServices::openUrl(QUrl(discord));
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Wavepay network", "", count));
}
...
void BitcoinGUI::gotoMarkets()
{
    marketsAction->setChecked(true);
    centralStackedWidget->setCurrentWidget(markets);
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Wavepay address or malformed URI parameters."));
    }
