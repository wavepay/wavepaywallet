...
{
    // Convert wavepay:// to wavepay:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("wavepay://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "wavepay:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Wavepay.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Wavepay.lnk
...
{
    return GetAutostartDir() / "wavepay.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a wavepay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Wavepay\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the wavepay app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add wavepay app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Wavepay-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  wavepay-qt [" + tr("command-line options") + "]                     " + "\n";
