...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
	...
	ui->assetlabel->setText(Qpricebtc + " BTC");
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeWavepayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeWavepayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeWavepayAmount = strAnonymizeWavepayAmount.remove(strAnonymizeWavepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWavepayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeWavepayAmount*COIN) nMaxToAnonymize = nAnonymizeWavepayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeWavepayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeWavepayAmount));
        strAnonymizeWavepayAmount = strAnonymizeWavepayAmount.remove(strAnonymizeWavepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWavepayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
