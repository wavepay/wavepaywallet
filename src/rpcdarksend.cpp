...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <wavepayaddress> <amount>\n"
            "wavepayaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <wavepayaddress> <amount>\n"
            "wavepayaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in wavepay.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in wavepay.conf\n"
                ...
                "  vote-many    - Vote on a Wavepay initiative\n"
                "  vote         - Vote on a Wavepay initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CWavepaycoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "vote-many")
    {
        ...
        if (params.size() != 2)
            throw runtime_error("You can only vote 'like' or 'dislike'");

        std::string vote = params[1].get_str().c_str();
        if(vote != "like" && vote != "dislike") return "You can only vote 'like' or 'dislike'";
        int nVote = 0;
        if(vote == "like") nVote = 1;
        if(vote == "dislike") nVote = -1;
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Wavepay initiative (can be additionally filtered, partial match)\n"
    ...
    if (strMode == "rank") {
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                {
                    if(mn.nVote == -1) strStatus = "dislike";
                    if(mn.nVote == 1) strStatus = "like";
                }
