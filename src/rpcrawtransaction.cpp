...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CWavepaycoinAddress(addr).ToString());
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CWavepaycoinAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Wavepay address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address))
        {
            entry.push_back(Pair("address", CWavepaycoinAddress(address).ToString()));
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "WVP decode failed");
    }
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CWavepaycoinAddress(script.GetID()).ToString()));
    return r;
}
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++)
    {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, mergedTx, i, STWVPARD_SCRIPT_VERIFY_FLAGS, 0))
            fComplete = false;
    }
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        if (!AcceptToMemoryPool(mempool, tx, true, NULL))
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "WVP rejected");
    }
