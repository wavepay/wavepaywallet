...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        // test randomness
        {
            ...
            int fails = 0;
            for (int i = 0; i < RWVPOM_REPEATS; i++)
            {
                // selecting 1 from 100 identical coins depends on the shuffle; this test will fail 1% of the time
                // run the test RWVPOM_REPEATS times and only complain if all of them fail
            ...
            BOOST_CHECK_NE(fails, RWVPOM_REPEATS);
